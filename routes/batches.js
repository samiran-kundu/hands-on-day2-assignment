const stoyys=require('../makedatabase').Batch
...
routee.route("/:id").get((req,res)=>{
    ...
    stoyys.findOne({where:{ batchid :req.params.id}}).
    then(result=>{
        if(result){
            res.json(result);
        }
...
.put((req,res)=>{
    ...
    stoyys.update(req.body,{where :{batchid: req.params.id}})
    .then(result=>{stoyys.findOne({where:{batchid:req.params.id}}).then(ress=>{res.send(ress);console.log(ress)})})
